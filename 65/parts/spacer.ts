...
import type { Rect, Side } from '@renderer/types'
...
export const buildSideWalls2D = (side: Side, width = WALL_WIDTH) => sideWalls(side.caseOutline, width)

export const buildSpacers2D = (side: Side, width = WALL_WIDTH) => subtract(buildSideWalls2D(side, width), union(...mountHoleCuts(side.mountHoles)))
