...
import { buildTopFdm3D, buildPlateFdm3D, buildBottomFdm3D, fdmOutline, untiltGeom } from '../parts/case-fdm'
...
export const buildPrintParts = (scaleFactor: number): PrintPart[] =>
    (Object.keys(SIDES) as (keyof typeof SIDES)[]).flatMap((key) => {
        ...
        const parts = [
            ...
            { name: `${key}-plate`, geom: dropToBed(untiltGeom(buildPlateFdm3D(side))) },
