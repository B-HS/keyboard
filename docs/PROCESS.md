...
> 최종 갱신: 2026-07-14 · 브랜치 **`keypad-17`** · **RMK BLE 펌웨어 완성 — keypad-17 실기 가동**: `keypad-17/firmware/`(rmk 0.8 + esp-hal 1.0, BLE "keypad17") 17키 전량 실기 검증 완료. 진단 펌웨어(probe)로 실배선 확정(COL=3·2·1·0 / ROW=21·20·10·5·6 — GPIO2·8·9 는 보드 풀업이라 ROW 불가 → 2가닥 재납땜), 2u 키 아랫행 배선(+=(2,3)/Enter=(4,3)), "−" 다이오드 역방향 수리, storage 옛 keymap 영속화 함정 해결(erase-region 0x3F0000). 운용 지식 `docs/memory/rmk-esp32c3.md`. 케이스·조립 전수검사(아래 m·n 항목)와 부트스트랩은 커밋 `8e139f4`·`9d28c31` 반영.
> 이전: 2026-07-12 · **keypad-17 부트스트랩**(`8e139f4`·`9d28c31`): 3D print + handwire(PCB 없음) 17키 키패드. 구 split 프로젝트(`65/`·`keypad/`) 제거(히스토리는 `split` 브랜치 보존), 공유 인프라(뷰어·설정·CI) 중립화. split 브랜치는 ESP32 크래들 정합 `a171b39` 반영(원격 push 완료). **아래 §0~§9의 monorepo·65·keypad 서술은 split 기준 이력** — keypad-17 구조 확정 후 재작성 예정.
...
- [x] o. **RMK 펌웨어 (2026-07-12 착수, 납땜 완료 상태)** — `keypad-17/firmware/` (rmkit 0.0.21 create, rmk 0.8 + esp-hal 1.0 no_std, target riscv32imc, BLE only `usb_enable=false`). 매트릭스는 raw kicad_pcb net 파싱으로 확정: 5×4 col2row(다이오드 cathode pad1→ROW 전수 확인), 조인트 net `N_R{r}_C{c}`로 2u 3개 포함 17키 전량 매핑(빈칸 (2,3)·(4,1)·(4,3)=`No`). 핀: COL0-3=GPIO0-3(사용자 기억), ROW0-4=**GPIO21·20·10·9·8 가정**("끝번호부터 5개" — 순서는 페어링 테스트로 확정 예정). keymap 표준 넘패드 + 예비 레이어, vial.json(2u h/w 반영). 빌드 1m00s 성공 → espflash 4.4로 플래시 완료(esp32c3 rev0.4, app 794KB), 부팅 로그: storage 64KB@0x3F0000·BLE 광고 개시·키 리포트 송신 확인. 주의 후보였던 GPIO9/8 스트랩 문제가 실제로 발현("." 연타) → 아래 o-2 진단으로 해결.
- [x] o-2. **매트릭스 실배선 진단·수정 (2026-07-12)** — 사용자 아이디어(행/열 한 줄씩 활성화)로 진단 펌웨어 `src/bin/probe.rs` 제작(13핀 Flex 순회 스캔: 각 핀 출력 HIGH→나머지 풀다운 입력 감지, 유휴 STUCK 검사, 다이오드 방향으로 COL/ROW 자동 판별). 발견: ①**GPIO2·8·9 외부 풀업(스트랩·BOOT·LED)로 유휴 STUCK-HIGH** → 풀다운 행 입력 불가(RMK matrix.rs가 set_high/is_high 하드코딩 — 소스 확인, 우회 불가) → "." 연타 원인 = ROW3·4(GPIO9·8) 상시 눌림 ②실배선 확정: COL0-3=GPIO **3·2·1·0**(역순), ROW0-2=GPIO21·20·10 ③**2u 세로키(+·Enter)는 아랫행 라인에 배선** → 매트릭스 (2,3)·(4,3) ④"−" 키 개별 접촉 불량. 수정: **행 와이어 GPIO9→5, GPIO8→6 재납땜**(사용자, 5·6 유휴 확인 후 승인) → 재검증으로 1·2·3(GPIO5행)·0·.·Enter(GPIO6행) 전량 클린 매핑. keyboard.toml 확정(row=[21,20,10,5,6], col=[3,2,1,0], + (2,3)/Enter (4,3)), vial.json 좌표 갱신, RMK 재빌드 완료.
- [x] o-3. **"−" 키 수리 + RMK 가동 (2026-07-12 완료)** — "−" 원인 = **다이오드 역방향/불량**(직결 시 도통·다이오드 장착 시 무반응으로 확정, probe 라이브 뷰 `watch-probe.sh` 로 사용자가 직접 사전 방향 테스트 후 납땜). 중간 사고: 플래시 중단으로 앱 손상·부트루프(`invalid segment length`) → probe 재플래시로 복구. RMK 플래시 후 +·Enter 가 `No` 로 뜨는 문제 = **storage 가 첫 플래시의 옛 keymap 을 영속화**(0x3F0000, 재플래시로 안 지워짐) → `espflash erase-region 0x3F0000 0x10000` + 재페어링으로 해결. **최종: BLE(keypad17) 17키 전량 정상 동작(사용자 확인).** 운용 지식 `docs/memory/rmk-esp32c3.md` 신설(핀 제약·storage 함정·probe 진단법·플래시 팁). 커밋 대기.
- [~] j. 실물 출력·조립·핸드와이어·**실사용 가동 완료**(2026-07-12–14, 펌웨어 o-3 기준 BLE 17키 동작). 미결: QA 체크리스트(`keypad-17-assembly-audit.md` §3) 세부 항목 체크(USB 케이블 분리 시 모듈 이탈 여부 등), plate 중앙(0키 부근) 처짐 시 rest post 추가, 핸드와이어 가이드 문서
