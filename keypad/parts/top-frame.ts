...
const cellHeight = (v: number) => v * KEY_PITCH - KEYCAP_GAP + 2 * LATERAL_CLEARANCE
...
export const buildOpening = (side: Side) => {
    const cells = union(...side.switches.map((s) => translate([s.x, s.y, 0], rectangle({ size: [cellWidth(s.u), cellHeight(s.vSpan ?? 1)] }))))
