...
const cellSpan = (units: number) => units * KEY_PITCH - KEYCAP_GAP + 2 * LATERAL_CLEARANCE
...
export const buildOpening = (side: Side) => {
    const cells = union(...side.switches.map((s) => translate([s.x, s.y, 0], rectangle({ size: [cellSpan(s.u), cellSpan(s.vSpan ?? 1)] }))))
    return union(...geom2.toOutlines(cells).map((outline) => geom2.fromPoints(filletOutline(outline, GAP_FILLET))))
}

export const buildTopFrame2D = (side: Side) => subtract(rect(side.caseOutline), buildOpening(side))
