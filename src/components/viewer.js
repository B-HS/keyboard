...
export const Viewer = ({ solids }) => {
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            if (solidsRef.current !== cachedSolids) {
                ...
                cachedEntities = entitiesFromSolids({}, ...cachedSolids);
        ...
        const onMove = (e) => {
            ...
            if (shift) {
                const updated = orbitControls.pan({ controls: orbitState, camera, speed: 300 }, [dx, dy]);
                orbitState = { ...orbitState, ...updated.controls };
                camera.position = updated.camera.position;
                camera.target = updated.camera.target;
            }
            else {
                const updated = orbitControls.rotate({ controls: orbitState, camera, speed: 1 }, [dx, dy]);
                orbitState = { ...orbitState, ...updated.controls };
            }
    ...
    return _jsx("canvas", { ref: canvasRef, className: "viewer-canvas" });
};
