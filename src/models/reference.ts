import { computeBounds, DEFAULT_STAB, type KeyPos, type Padding, type StabSpec } from './layout'
...
export type ReferenceSpec = {
    ...
    screwHoles: Array<{ x: number; y: number; radius: number }>
}

const num = '(-?\\d+(?:\\.\\d+)?)'

const matchOrThrow = (source: string, regex: RegExp, label: string): RegExpMatchArray => {
    const m = source.match(regex)
    if (!m) throw new Error(`reference parse failed: ${label}`)
    return m
}

const matchAll = (source: string, regex: RegExp): RegExpMatchArray[] => [...source.matchAll(regex)]

export const parseReference = (source: string): ReferenceSpec => {
    const thickness = parseFloat(matchOrThrow(source, new RegExp(`const THICKNESS = ${num}`), 'THICKNESS')[1])

    const ws = '\\s*'
    const sep = `,${ws}`

    const outlineMatch = matchOrThrow(
        source,
        new RegExp(
            `const outline = translate\\(${ws}\\[${num}${sep}${num}${sep}0\\]${sep}roundedRectangle\\(\\{${ws}size:${ws}\\[${num}${sep}${num}\\]${sep}roundRadius:${ws}${num}${ws}\\}\\)${ws},?${ws}\\)`,
        ),
        'outline',
    )

    const switchMatch = matchOrThrow(
        source,
        new RegExp(`const switch_shape = roundedRectangle\\(\\{${ws}size:${ws}\\[${num}${sep}${num}\\]${sep}roundRadius:${ws}${num}${ws}\\}\\)`),
        'switch_shape',
    )

    const stabMatch = matchOrThrow(
        source,
        new RegExp(`const stab_pad = roundedRectangle\\(\\{${ws}size:${ws}\\[${num}${sep}${num}\\]${sep}roundRadius:${ws}${num}${ws}\\}\\)`),
        'stab_pad',
    )

    const holeRegex = new RegExp(
        `const hole\\w+ = translate\\(${ws}\\[${num}${sep}${num}${sep}0\\]${sep}circle\\(\\{${ws}radius:${ws}${num}${ws}\\}\\)${ws},?${ws}\\)`,
        'g',
    )
    ...
    if (screwHoles.length === 0) throw new Error('reference parse failed: no screw holes found')
...
export const deriveDefaults = (ref: ReferenceSpec, keys: KeyPos[]): DerivedDefaults => {
    const zeroPadding: Padding = { top: 0, bottom: 0, left: 0, right: 0 }
...
export const formatSummary = (ref: ReferenceSpec, derived: DerivedDefaults): string => {
