...
export const DEFAULT_LOLIN_PARAMS = {
    pcbWidth: 25.4,
    pcbLength: 34.3,
    pcbThickness: 1.6,
    componentHeight: 2.0,
...
export const buildLolin = (plateBounds, caseP, p = DEFAULT_LOLIN_PARAMS) => {
    ...
    const pcb = translate([pcbCenterX, pcbCenterY, pcbCenterZ], cuboid({ size: [p.pcbWidth, p.pcbLength, p.pcbThickness] }));
    const componentsCenterZ = pcbCenterZ + p.pcbThickness / 2 + p.componentHeight / 2;
    const components = translate([pcbCenterX, pcbCenterY, componentsCenterZ], cuboid({ size: [p.pcbWidth - 4, p.pcbLength - 4, p.componentHeight] }));
