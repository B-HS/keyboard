...
import { buildPerfBoard, buildSlideSwitch, buildStabilizers, buildFootPads, buildBatteryCover, buildBatteries, buildBatteryContacts, } from './accessories';
import { normalizeSwitch, placeSwitches, DEFAULT_SWITCH_ORIENT } from './switch';
import { buildKeycapsForKeys, DEFAULT_KEYCAP_ORIENT } from './keycap';
import { normalizeStabilizer, placeStabilizers, DEFAULT_STABILIZER_ORIENT } from './stabilizer';
const { colorize } = colors;
const { cuboid } = primitives;
const { translate, rotateX } = transforms;
const PHONE_SIZE = [77.6, 160.7, 7.85];
const PHONE_GAP = 10;
const WOBKEY_ZEN65_SIZE = [315, 112, 28];
const WOBKEY_ZEN65_GAP = 15;
const CASE_TOP_COLOR = [0.77, 0.77, 0.77, 1];
const CASE_BOTTOM_COLOR = [0.77, 0.77, 0.77, 1];
const SWITCH_COLOR = [0.77, 0.77, 0.77, 1];
const LOLIN_COLOR = [0.1, 0.5, 0.2, 1];
const PERFBOARD_COLOR = [0.75, 0.6, 0.3, 1];
const SLIDE_COLOR = [0.7, 0.7, 0.7, 1];
const STAB_COLOR = [0.77, 0.77, 0.77, 1];
const KEYCAP_COLOR = [0.77, 0.77, 0.77, 1];
const FOOTPAD_COLOR = [0.1, 0.1, 0.1, 1];
const BATTERY_COVER_COLOR = [0.77, 0.77, 0.77, 1];
const BATTERY_COLOR = [0.85, 0.75, 0.3, 1];
const CONTACT_COLOR = [0.8, 0.8, 0.85, 1];
const PHONE_COLOR = [0.4, 0.6, 0.8, 0.6];
const WOBKEY_COLOR = [0.75, 0.45, 0.35, 0.6];
export const buildSolids = (visibility, params, switchGeom = null, switchOrient = DEFAULT_SWITCH_ORIENT, keycapOrient = DEFAULT_KEYCAP_ORIENT, stabilizerGeom = null, stabilizerOrient = DEFAULT_STABILIZER_ORIENT) => {
    const solids = [];
    ...
    if (visibility.stabilizers) {
        if (stabilizerGeom) {
            ...
            if (stabs)
                solids.push(colorize(STAB_COLOR, applyPlateTransform(stabs)));
        }
    }
    ...
    if (visibility.phone) {
        ...
        const phone = translate([phoneCenterX, phoneCenterY, phoneCenterZ], cuboid({ size: PHONE_SIZE }));
    ...
    if (visibility.wobkeyZen65) {
        ...
        const ref = translate([centerX, centerY, centerZ], cuboid({ size: WOBKEY_ZEN65_SIZE }));
